      ...
      <div className="max-w-4xl mx-auto px-4 py-16">
        ...
        {items.length === 0 ? (
          <div className="mt-6 text-gray-600">
            <p>{t.cart.empty}</p>
            <Link href="/products" className="text-blue-600 hover:text-blue-700 mt-2 inline-block">{t.cart.browseProducts}</Link>
          </div>
        ) : (
          <div className="mt-6 space-y-4">
            {items.map((i, idx) => (
              <div key={idx} className="flex items-center gap-4 border rounded-lg p-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-sm text-gray-600">{t.cart.quantity}: {i.quantity}</div>
                  {i.selectedOptions && (
                    <div className="text-sm text-gray-600">{t.cart.options}: {Object.entries(i.selectedOptions).map(([k,v]) => `${k}: ${v}`).join(', ')}</div>
                  )}
                </div>
                <div className="text-gray-900 font-semibold">{formatPrice(i.price * i.quantity, locale)}</div>
                <button
                  type="button"
            ...
            <div className="flex items-center justify-between border-t pt-4 mt-4">
              <div className="text-xl font-semibold">{t.cart.total}: {formatPrice(total, locale)}</div>
              <div className="flex items-center gap-3">
                <Link href="/products" className="px-4 py-2 rounded-md border hover:bg-gray-50">{t.cart.continueShopping}</Link>
                <button type="button" onClick={handleClear} className="px-4 py-2 rounded-md bg-red-600 text-white hover:bg-red-500">{t.cart.clearCart}</button>
              </div>
            </div>
          </div>
        )}
